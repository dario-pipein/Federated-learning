...
MATCHED_PARTS = {
    ...
    "TORONTO":  ([1, 17], [2, 16], [3, 15], [4, 14], [5, 13], [6, 12], [7, 11], [8, 10],
             [18, 27], [19, 26], [20, 25], [21, 24], [22, 23],
             [32, 36], [33, 35],
             [37, 46], [38, 45], [39, 44], [40, 43], [41, 48], [42, 47],
             [49, 55], [50, 54], [51, 53], [62, 64], [61, 65], [68, 66], [59, 57], [60, 56])}
...
def crop(img, center, scale, output_size, rot=0):
    ...
    if sf < 2:
        ...
        if new_size < 2:
            ...
            img = skimage.transform.resize(img, [new_ht, new_wd])  #MAX deprecated: scipy.misc.imresize(img, [new_ht, new_wd])  # (0-1)-->(0-255)
    ...
    if not rot == 0:
        # Remove padding
        new_img = scipy.ndimage.rotate(new_img, rot) #MAX: deprecated: scipy.misc.imrotate(new_img, rot)
        new_img = new_img[pad:-pad, pad:-pad]
    new_img = skimage.transform.resize(new_img, output_size)  #MAX: deprecated: scipy.misc.imresize(new_img, output_size)
    return new_img
